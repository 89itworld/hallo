...
class CouponsController extends AppController {
	...
	*/
	...
	$coupon_actv_code = str_replace("|",$user_all_data['delimiter'],$value['activation_code']);
		 ...
		 //if(1){
			...
			if(!empty($coupon_id) && !empty($a_code)){
				...
				foreach($Productdata as $k => $val){
					...
					if(!empty($val)){
						$searchText = array('{VENDOR_NAME}','{VOUCHER}','{COUPON_CODE}','{ACTIVATION_CODE}','{USERNAME}', '{ADDRESS}', '{CITY}', '{POSTNUMBER}', '{DATE}', '{TIME}','{n}');
						$replaceText = array($Vendordata[$validvendorID],$Categorydata[$validcategoryID],$coupon_id,str_replace("|",$user_all_data['delimiter'],$a_code),$user_all_data['u_name'],$user_all_data['UserProfile']['address'],$user_all_data['UserProfile']['city'],$user_all_data['UserProfile']['post_number'],$current_date,$current_time,'');
