...
class UsersController extends AppController {
			...
			* which will return true if it validates and false if it doesn�t:
			...
			if(count($errors) == 0){
				if($this->Session->read("Auth.User.role_id") == "2"){
					if($this->Session->read("Auth.User.id") != $this->data['User']['parent_id']){
						$this->Session->setFlash("Please enter valid data", 'message/red');
	...
	*/
	...
	function admin_edit_user($user_id = null){
		...
		if(!empty($this->data)){
			...
			if($this->Session->read("Auth.User.role_id") == "2"){
				if($this->Session->read("Auth.User.id") != $data['User']['parent_id']){
					$this->Session->setFlash("Please enter valid data", 'message/red');
					$this->redirect(array('controller'=>'users','action'=>'user_listing','admin'=>true));exit();
				}
			}
			...
			if(count($errors) == 0){
				...
				if($data['User']['role_id'] == "2"){
					$u_id = $data['User']['u_id'];
					$this->User->updateAll(array("User.u_id"=>"'$u_id'"),array("User.parent_id"=>$data['User']['id']));
				}
	...
	*/
	function admin_user_permission($r_id = null,$type = null){
		$this->layoout = "";
		if($type == ""){
			...
			$user_ids = $this->User->find("first",array("recursive"=>-1,"fields"=>array("User.id","User.u_id"),"conditions"=>array("User.id"=>$r_id,"User.is_active"=>"1","User.is_deleted"=>"0")));
			$this->set('user_ids',$user_ids);
			$this->set('u_type',$type);
		}
	...
	function admin_edit_role($role_id = null){
		...
		if(!empty($this->data)){
			...
			if(count($errors) == 0){
				...
				$user_datas = $this->User->find("list",array("fields"=>array("User.id","User.id"),"conditions"=>array("User.is_deleted"=>"0","User.role_id"=>$role_id)));
				$user_datas = array_keys($user_datas);
				...
				$role_permission_ids = $this->RolePermission->find("list",array("fields"=>array("RolePermission.permission_id","RolePermission.id"),"conditions"=>array("RolePermission.is_deleted"=>"0","RolePermission.role_id"=>$role_id)));
				$role_permission_ids = array_keys($role_permission_ids);
				
				$this->RolePermission->updateAll(array("RolePermission.is_active"=>"'0'"),array("RolePermission.role_id"=>$role_id));
				
				$this->UserPermission->updateAll(array("UserPermission.is_active"=>"'0'"),array("UserPermission.user_id"=>$user_datas));
				$permission_data['RolePermission']['role_id'] = $role_id;
				foreach($this->data['RolePermission']['permission_id'] as $permisson_id){
					if(in_array($permisson_id,$role_permission_ids)){
						
						$this->UserPermission->updateAll(array("UserPermission.is_active"=>"'1'"),array("UserPermission.permission_id"=>$permisson_id,"UserPermission.user_id"=>$user_datas));
						
						$this->RolePermission->updateAll(array("RolePermission.is_active"=>"'1'"),array("RolePermission.permission_id"=>$permisson_id,"RolePermission.role_id"=>$role_id));
	...
	*/
	...
	/*
	* Function for generating dealer report csv
	*/
