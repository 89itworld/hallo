...
class CouponsController extends AppController {
	...
	*/
 	...
 	function admin_coupon_pull(){
		...
		if($user['role_id'] == 1) {
					...
					App::import("Model","Product");
					$this->Product = new Product();
					$Productdata = $this->Product->find('all',array('fields' => array('Product.id','Product.title','Product.product_code'),'conditions' => array('Product.is_active' => '1','Product.is_deleted' => '0'),'recursive' => 0   ));
					...
					$tmp_Productdata = Set::extract('{n}.Product.product_code',$tmp_Productdata);   
				 	...
				 	App::import("Model","Coupon"); 
					$this->Coupon = new Coupon(); 
					$Coupontdata = $this->Coupon->find('all',array('fields' => array('product_code','COUNT(product_code) AS products_count'),'conditions' => array('Coupon.product_code' => $tmp_Productdata,'Coupon.is_sold' => '0' ) , 'group' => array('Coupon.product_code' ) ));
					...
					foreach($Productdata as $key=>$value){ 
							...
							if(!empty($product_Coupons) && isset($product_Coupons[$value['product_code']]) ){
								 ...
								 $value=$value['title']." {".$product_Coupons[$value['product_code']]."}";
							}else{ 
									$value=$value['title']." {0}";
							}
	...
	function admin_bulksalecoupon($product_id=null,$counter=null){ 
		...
		if($this->RequestHandler->isAjax() || (!empty($product_id) && !empty($counter) && is_numeric($product_id) && is_numeric($counter))){
			...
			if(!empty($this->data)){
				...
				$this->Batch = new Batch();
				$batch_data = $this->Batch->find("list",array("fields"=>array("Batch.id","Batch.name"),"conditions"=>array("Batch.is_deleted"=>"0","Batch.is_active"=>"1"),"order"=>"Batch.name asc"));
				$product_title = $this->Product->field('title',array("Product.id"=>$product_id));
				...
				if(isset($this->data['export_to_csv']) && ($this->data['export_to_csv'] == "csv")){
					$this->generate_coupon_report_csv($coupon_data,$batch_data,$product_title);
					
				}
				if(isset($this->data['export_to_pdf']) &&  ($this->data['export_to_pdf']== "pdf")){
					$this->generate_coupon_report_pdf($coupon_data,$batch_data,$product_title);
				}
		...
		//if(!empty($product_id) && !empty($counter) && is_numeric($product_id) && is_numeric($counter)){
			...
			if(!empty($ProductValid) ){
				...
				$ISCouponAvaliable=$this->Coupon->find('list',array('fields'=>array('id','coupon_id'),'conditions' => array('Coupon.product_code' => $product_id,'Coupon.is_sold' => '0'),'limit' => $counter) );
				$available=count($ISCouponAvaliable);
				 
				if($available == $counter){
					
					if($this->Coupon->updateAll(array("Coupon.is_pulled"=>"'1'","Coupon.is_sold"=>"'1'"),array("Coupon.coupon_id"=>$ISCouponAvaliable))){
	...
	*/
	...
	function generate_coupon_report_pdf($data = null,$b_data=array(),$p_title=null){
		App::import('Vendor','tcpdf/tcpdf');
			...
			foreach($data as $result){
                ...
                $tcpdf->Output(str_replace(" ","_","$p_title")."_".date("d_M_Y_h_i_s_a").'.pdf', 'D');
		
		//for save into folder
		$directory_path = $this->create_directory("pull_coupon_sale_pdf");
		$tcpdf->Output($directory_path.str_replace(" ","_","$p_title")."_".date("d_M_Y_h_i_s_a").'.pdf', 'F');
		exit();
	}
				...
				if(is_array($Dealerlimit) && !empty($Dealerlimit) && $Dealerlimit['action'] && $Dealerlimit['price_coupon_limit']!=0 ){ 
					...
					if((!empty($vendor_id)) && (!empty($category_id)) ){ 
						...
						if(($validvendorID > 0 ) && ($validcategoryID > 0)){    
								App::import("Model","Coupon");
								$this->Coupon = new Coupon(); 
							 	$Coupon = $this->Coupon->find('first',array("conditions"=>array('Coupon.vendor_id'=> $vendor_id,'Coupon.category_id' =>$category_id ,'Coupon.is_sold'=> '0')));
							 	if(!empty($Coupon) && is_array($Coupon)){ 
										...
										if(!empty($Productdata) && is_array($Productdata)){
											...
											$soldcoupon=array('Coupon'=> array('id' => $sold_CouponID ,'is_sold'=> '1'));
