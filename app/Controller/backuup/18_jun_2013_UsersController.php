...
class UsersController extends AppController {
	...
	*/
	...
	function admin_overall_trends(){
		...
		$ulist=$this->User->find('all',array('conditions'=>array('User.role_id'=>2),'fields'=>array('User.id','CONCAT(UCASE(SUBSTRING(UserProfile.first_name, 1, 1)),LCASE(SUBSTRING(UserProfile.first_name, 2))) as name'),'recursive'=>0));
		...
		$vendor_data = $this->Vendor->find("all",array("recursive"=>-1,"fields"=>$field,"conditions"=>array("Vendor.is_deleted"=>"0","Vendor.is_active"=>"1"),"order"=>"Vendor.name asc"));
		//$this->set("categories_list",$this->find_categories_list());
		...
		foreach($vendor_list as $key=>$value){
			$temp=array();
$temp = $this->Coupon->find('list',array('conditions'=>array('Coupon.vendor_id'=>$key,'Coupon.is_active'=>'1','Coupon.is_deleted'=>'0'),'fields'=>array('Coupon.id','Coupon.category_id')));
			$handled_cat['vendor'][$key]['key']=$key;
			$handled_cat['vendor'][$key]['name']=$value;
			$handled_cat['vendor'][$key]['categories'][]=array_unique($temp);
		}
		
		$categories_list = $this->find_categories_list();
		$large_array=array();
		foreach($handled_cat as $lkey =>$lvalue){
			foreach($lvalue as $kkey=>$kvalue){
				if(!empty($kvalue['categories'][0])){$i=0;
					foreach($kvalue['categories'][0] as $key=>$value){
						$remain = $this->Coupon->find('count',array('conditions'=>array("Coupon.is_sold"=>"0","is_active"=>"1","is_deleted"=>"0","Coupon.expire_date >"=>date("Y-m-d"),"Coupon.category_id"=>$value)));
						
						$large_array[$kvalue['key']]['name'] = $kvalue['name'];
						$large_array[$kvalue['key']]['catid'][$i]['id_cat'] = $categories_list[$value];
						
						if(empty($remain)){
							$large_array[$kvalue['key']]['catid'][$i]['remain'] = 0;
						}else{
							$large_array[$kvalue['key']]['catid'][$i]['remain'] = $remain;
						}
						$i++;
					}
				}else{
					$large_array[$kvalue['key']]['name']=$kvalue['name'];
					$large_array[$kvalue['key']]['catid']=array();	
				}
			}
		}
