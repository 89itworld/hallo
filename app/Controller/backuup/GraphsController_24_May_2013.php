...
class GraphsController extends AppController {
	...
	switch ($case) {
		...
		$dealer_data = $this->User->query('SELECT c.user_id , concat( c.first_name, "-", c.last_name ) AS name FROM users as a INNER JOIN user_profiles as c ON c.user_id = a.id WHERE a.`role_id` =2 AND a.`id` IN('.$dealer.') order by c.first_name');
