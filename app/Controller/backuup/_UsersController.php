...
class UsersController extends AppController {
	...
	*/
	...
	function admin_overall_trends(){
		...
		$product_list =  $this->Product->find('list',array('recursive'=>-1,'fields'=>array('Product.product_code','Product.title'),'conditions'=>array('Product.is_deleted'=>'0','Product.is_active'=>'1'),'order'=>'Product.title'));
		
		$large_array=array();
		$i = 0;
		foreach($product_list as $k1=>$v1){
			$remain = $this->Coupon->find('count',array('conditions'=>array("Coupon.is_sold"=>"0","is_active"=>"1","is_deleted"=>"0","Coupon.expire_date >"=>date("Y-m-d"),"Coupon.product_code"=>$k1)));
			$large_array[$i]['name'] = $v1;
			if(empty($remain)){
