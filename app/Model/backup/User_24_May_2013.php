...
class User extends AppModel {
	...
	*/
	...
	function __is_userid_valid($d_id = null,$user_id = null){
		...
		if($user_id == null){
			$data = $this->find("count",array("conditions"=>array("User.u_id"=>$d_id)));
			if($data == 0){
				return true;
			}
		}else{
			$data = $this->find("count",array("conditions"=>array("User.u_id"=>$d_id,'User.id'=>$user_id)));
