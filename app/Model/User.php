...
class User extends AppModel {
	...
	*/
	...
	function validate_add_user($postArray = null){
		...
		if(isset($postArray['UserProfile']['address'])){
			if(trim($postArray['UserProfile']['address']) == ""){
				$this->errMsg['address'][] = ERR_ADDRESS_EMPTY."\n";
			}
		}
		if(isset($postArray['UserProfile']['city'])){
			if(trim($postArray['UserProfile']['city']) == ""){
				$this->errMsg['city'][] = ERR_CITY_EMPTY."\n";
			}
		}
		if(isset($postArray['UserProfile']['post_number'])){
			if(trim($postArray['UserProfile']['post_number']) == ""){
				$this->errMsg['post_number'][] = ERR_POST_NUMBER_EMPTY."\n";
			}
		}
	...
	function _check($post_array = null){
		...
		}elseif(!($this->__is_userid_valid($post_array['u_id'],$u_id,$check_subdealer))){
			$this->errMsg['u_id'][] = LBL_DEALERID_ALREADY_EXIST."\n";
		}
			
		if(isset($post_array['u_name'])){ 
			if(trim($post_array['u_name']) == ""){
				$this->errMsg['u_name'][] = ERR_USERNAME_EMPTY."\n";
			}elseif(!($this->__is_username_valid($post_array['u_name'],$u_id))){
				$this->errMsg['u_name'][] = ERR_USERNAME_ALREADY_EXIST."\n";
			}
		}
	...
	*/
	...
	function __is_userid_valid($d_id = null,$user_id = null,$check_sub_dealer=null){
		if($check_sub_dealer == 0){
			if($user_id == null){
		...
		}elseif($check_sub_dealer == 1){
			return true;	
		}elseif($check_sub_dealer == 2){
			if($user_id == null){
				$data = $this->find("count",array("conditions"=>array("User.u_id"=>$d_id,"User.is_deleted"=>"0")));
				if($data == 0){
					return true;
				...
				$data = $this->find("count",array("conditions"=>array("User.u_id"=>$d_id,'User.id'=>$user_id,"User.is_deleted"=>"0","User.is_dealer"=>"1")));
				if($data == 1){
					return true;
				}elseif($data == 0){
					$data = $this->find("count",array("conditions"=>array("User.u_id"=>$d_id,'User.id !='=>$user_id,"User.is_deleted"=>"0","User.parent_id"=>"0")));
					if($data != 0){
						return false;
					}else{
						return true;
	...
	/*To Check uniqueness of username*/
	function __is_username_valid($uname = null,$user_id = null){
		
		if($user_id == null){
			$data = $this->find("count",array("conditions"=>array("User.u_name"=>$uname,"User.is_deleted"=>"0")));
			if($data == 0){
				return true;
			}
		}else{
			$data = $this->find("count",array("conditions"=>array("User.u_name"=>$uname,'User.id'=>$user_id,"User.is_deleted"=>"0")));
			if($data == 1){
			    return true;
			}else{
			    $data = $this->find("count",array("conditions"=>array("User.u_name"=>$uname,'User.id !='=>$user_id,'User.is_deleted'=>0)));
