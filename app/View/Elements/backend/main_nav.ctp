...
<nav id="main-nav">
		<ul class="container_12">
				<?php if($this->Session->read("Auth.User.role_id") == "3"){
						//it is used only to see my profile for dealer
						if(isset($my_profile_permissions) && ($my_profile_permissions == true)){
						$curr_class = '';
						if(($this->params['controller'] == 'dashboards') && ($this->params['action'] == 'admin_my_profile')){
							$curr_class = 'current';	
						}
				?>
						<li onclick="window.location='<?php echo HTTP_HOST?>admin/dashboards/my_profile'" title="" class="<?php echo $curr_class;?> users"> 
	<a href="/hallo/admin/dashboards/my_profile"></a>
						<ul>
								<li style="margin:0px 0.6em 0px 0px !important;padding:0px !important;" class="current ">
										<a href="/hallo/admin/dashboards/my_profile">My Profile</a>								  </li>	
						</ul>
			
						</li> 
				<?php } }
				//$main_tab = array("1"=>"home","2"=>"users","3"=>"role","4"=>"vendor","5"=>"voucher","6"=>"product","7"=>"coupon","8"=>"couponsale","9"=>"stats");
				$main_tab = array("1"=>"home","2"=>"users","3"=>"comments","4"=>"settings","6"=>"settings","7"=>"write","8"=>"write","46"=>"stats","60"=>"backup","72"=>"pull");//"4"=>"medias"
				$main_tab_title = array("1"=>"Dashboard","2"=>"User Management","3"=>"Role Management","4"=>"Product Management","6"=>"Product Management","7"=>"Coupon Management","8"=>"Sale Coupon Management","46"=>"Report Management","60"=>"Settings","72"=>"Pull Coupons");
				...
				?>
						...
						?>
								<ul>
										...
										?>
										<li class='check_sub_menu <?php echo $margin_menu." ".$sub_current_class;?>' style='<?php echo $margin_menu;?>'>
										<?php
										echo $this->Html->link(ucfirst($v1['title']),array("controller"=>$v1["controller"],"action"=>$v1["action"],"admin"=>true),array("escape"=>false));
										?>
										</li>	
								<?php }elseif((($this->params['controller']== "coupons") && ($this->params['action'] = "admin_upload_coupon_csv")) || (($this->params['controller'] == "coupons") && ($this->params['action'] = "admin_coupon_pull")) || (($this->params['controller']== "coupons") && ($this->params['action'] = "admin_coupon_pull_history")) || (($this->params['controller']== "batches") && ($this->params['action'] = "admin_listing"))){?>
									<script>	
										$(document).ready(function(){
												$(".check_sub_menu").children("a").find("COUPON MGMT");
												$( "li.check_sub_menu a").each(function() {
														
														if($(this).text() == "Coupon Mgmt"){
																
																$(this).parent('li').addClass("current");
																$(this).parent('li').attr("style","<?php echo $margin_menu; ?>");			
														}	
												});
										});
									</script>
