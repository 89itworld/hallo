    ...
    <section class="grid_8" style="width:100%;">-->
        <div class="block-border">
            <?php echo $this->element('backend/server_error');
                echo $this->Form->create('Vendor',array('url'=>array('controller'=>'vendors','action' => 'vendor_report', 'admin' =>true),'inputDefaults'=>array('div'=>false,'label'=>false),"type"=>"get",'id'=>'category_list', 'name' => 'category_list','class' => 'block-content form'));
		/*
		<h1>
		...
		*/ ?>
			...
			<p class="colx2-left">
				...
				<label for="stats-period">Vendor Name</label>
...
?>
	...
	<div class="no-margin"><table class="table" cellspacing="0" width="100%">
		...
		<?php echo '<span class="sortkey">'.$this->Paginator->sort( 'name','Vendor Name').'</span>'; ?> 
		</th>
		<th scope="col">
		<span class="column-sort">
		<?php
	    ...
	    <tbody>
	    <?php 
	    if(!empty($vendor_data)){foreach($vendor_data as $Record){ ?>
		<tr>
		    <td> 
			<?php echo ucfirst($Record['Vendor']['name']); ?>
		    </td>
		    <td> 
			<?php 
			    $status =array("Deactive","Active");
			    echo $status[$Record['Vendor']['is_active']];
			?>
		    </td>
		    <td> 
