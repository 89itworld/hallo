...
<div style="opacity: 1;" id="control-bar" class="grey-bg clearfix">
		<div class="container_12">
				<div class="float-right">
						...
						<?php
						$parmission_add_user = $this->Session->read('UserPermission');
						
						foreach($parmission_add_user as $k1=>$v1){
						    foreach($v1['SubPermission'] as $k2=>$v2){
							if(($v2['controller'] == "users") && ($v2['action'] == "admin_add_user") && ($this->Session->read("Auth.User.role_id") != "3")){
				...
				<div class="no-margin"><table class="table" cellspacing="0" width="100%">
						...
						?>
								<tr>
										...
										<td class="active_check"> 
											<?php 
											$status =($Record['User']['is_active'] == '1')?1:0;
											$check_status = "#";
											foreach($parmission_add_user as $k1=>$v1){
											    foreach($v1['SubPermission'] as $k2=>$v2){
												if(($v2['controller'] == "users") && ($v2['action'] == "admin_activate_bulk")){ 
												    $check_status_flag = 1;
												    $check_status = '/admin/users/activate_bulk/'.$status.'/'.ENCRYPT_DATA($Record['User']['id']).'/User';
												    break;
												}
											    }
											}
											...
											echo $this->Common->is_active($Record['User']['is_active'],$check_status);
										...
										<td class="table-actions">
											...
											foreach($parmission_add_user as $k1=>$v1){
												...
												if(($v2['controller'] == "users") && ($v2['action'] == "admin_edit_user") && ($this->Session->read("Auth.User.role_id") != "3")){
												    $i = 1;
												...
												if(($v2['controller'] == "users") && ($v2['action'] == "admin_delete_bulk") && ($this->Session->read("Auth.User.role_id") != "3")){
												    $i = 1;
